...
def validar_consulta(msj="Ingrese la consulta: "):
    while True:
         consulta = input(msj)
...
def consultar_envio(matriz2):
    ...
    if tipo_elegido == 1:
        ...
        lista_fecha = validar_consulta(msj="Ingrese la fecha a consultar: ")
        print(lista_fecha)
        print()
        encontrado = False
        if matriz2[4] == []:
            print("No hay pedidos cargados aun, pulse 1 para cargar un pedido nuevo")
        else:
        ...
        match int(opcion):
            case 1: #LISTAR TODo#
                ...
                print(f"Total de Pedidos: {total}")
                    ...
                    total_envios = len(matriz[0])  # cantidad total de pedidos cargados
