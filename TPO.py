            ...
            for linea in arch:
                if linea.strip() != "":
                    ...
                    if campos[1] == consulta:
                        ...
                        print("-"*100)
                        print(f"{campos[1]:^7} | {campos[2]:^15} | {campos[3]:^15} | {campos[4]:^15} | {campos[5]:^{25 if campos[5][ :9] == "devuelto" else 10}} | {campos[6]:^15}")
                        print("-"*100)
                        break
                    ultima_linea = linea
...
def cambiar_estado():
    ...
    try:
        ...
        if volver:
            ...
            while ingreso != "si" and ingreso != "no":
                ingreso = input("Ingreso invalido, ¿Desea continuar cambiando estados de envios? Si / No: ").lower()
            if ingreso == "no":
                pass
            else:
                cambiar_estado()
