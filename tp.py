...
def guardar_txt(sistema, path="pedidos.txt"):
    """Guarda la matriz en txt"""
    try:
        thompson = open(path, "wt")  
        for i in range(len(sistema[0])):
            d, m, y, hh, mm = sistema[4][i] 
            linea = (
                f"{sistema[0][i]};"            # código
                f"{sistema[1][i]};"            # cliente
                f"{sistema[2][i]};"            # dirección
                f"{sistema[3][i]};"            # estado
                f"{d:02d}/{m:02d}/{y} {hh:02d}:{mm:02d}\n"  # fecha/hora
            )
            thompson.write(linea)
        thompson.close()
        print(f"Guardado OK en {path}")
    except OSError as e:
        print("Error al guardar TXT:", e)

def guardar_txt_append(registro, path="pedidos.txt"):
    """At para cargar menos memoria"""
    try:
        thompson = open(path, "at")  # append
        d, m, y, hh, mm = registro[4]
        linea = (
            f"{registro[0]};"             # código
            f"{registro[1]};"             # cliente
            f"{registro[2]};"             # dirección
            f"{registro[3]};"             # estado
            f"{d:02d}/{m:02d}/{y} {hh:02d}:{mm:02d}\n"
        )
        thompson.write(linea)
        thompson.close()
        print(f"Linea agregada a {path}")
    except OSError as e:
        print("Error al guardar TXT (append):", e)
...
def agregar_envio(contador1, matriz1): #Contador sirve para q se hagan las iteraciones, las toma cuando llama a codigo_envio
    ...
    registro = [codigo2, cliente, direccion, estado, fecha_sin_formato]
    guardar_txt_append(registro)
    ...
    print("/".join(map(str, matriz1[4][contador1 - 1][:3])), end=" ")
...
def consultar_envio(matriz2):
    ...
    if matriz2[0] == []:
        ...
        if tipo_elegido == 'Por codigo':
            ...
            lista_fecha = validar_horario(msj="Ingrese la fecha a consultar (formato: DD/MM/AAAA hh:mm o DD-MM-AAAA hh:mm): ")
            print()
            encontrado = False
